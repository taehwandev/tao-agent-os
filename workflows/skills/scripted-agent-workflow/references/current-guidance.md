...
The wrapper commands themselves are a separate reading:
`workflows/skills/scripted-agent-workflow/references/evidence-wrappers.md`
covers each hook's invocation, what it writes, the gate signals it records, and
the output conventions.

They were split out because this reference was over the route's
mandatory-reading budget, which kept the whole of it out of required reading
and reachable only as a reference.
