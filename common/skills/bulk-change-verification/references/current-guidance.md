...
If a transform damaged formatting, restore only the task-owned hunks from an
explicitly captured pre-transform baseline, then rewrite the transform. Prefer
a targeted reverse patch or an explicit edit that preserves unrelated and
pre-existing user changes. Never reset the whole file to `HEAD` unless the file
was clean at task start and every current change in it is proven task-owned.

Do not try to regex-"fix" the formatting in place; the fix pass also touches
already-correct blocks and makes the damage worse. Do not assume an
auto-formatter can recover the file unless the repo actually has one
configured. If damaged task-owned hunks overlap changes that cannot be safely
attributed, stop instead of guessing which bytes to discard.
...
- The diff shows shifted indentation or damaged formatting in any file; restore
  only the attributable task-owned hunks and rewrite the transform before
  continuing.
- Damaged task-owned hunks cannot be separated safely from pre-existing or
  concurrent changes in the same file.
