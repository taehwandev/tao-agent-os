...
class RuntimeExecutionCapsuleBridgeTests(unittest.TestCase):
    ...
    def test_codex_approval_wait_is_bounded_and_evidence_first(self) -> None:
        ...
        for required_boundary in (
            ...
            "idempotent local command that normally completes immediately",
            "do not spend multiple minutes on repeated empty waits",
            "This recovery does not apply to non-idempotent or external writes",
    ...
    def test_agent_operating_guidance_preserves_non_git_rules_drift_recovery(self) -> None:
        ...
        self.assertIn("the project or rules root changed after", guidance)
        self.assertIn("real stale-input evidence", guidance)
        self.assertIn("Do not revert the concurrent change", guidance)
