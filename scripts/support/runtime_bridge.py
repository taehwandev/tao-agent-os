...
CODEX_APPROVAL_WAIT_BRIDGE_PHRASE = (
    ...
    "and target-state evidence still shows no side effect, interrupt the invocation, confirm termination, "
    "reconcile possible side effects, and use another supported path. This recovery does not apply to "
...
RUNTIME_START_BRIDGE_PHRASE = (
    ...
    "and ambiguity. Compute that fingerprint with agent-hook.py fingerprint, passing exactly the "
    "same --request, --continuation-scope, and classification flags the start call will use -- the "
    "binding covers that full intake, so a scope passed only to start makes the envelope describe a "
    "different request; the helper also prints an envelope skeleton and stays callable before any "
    "start. When the "
    "effective effect is git_write or higher, also pass a separately recorded "
