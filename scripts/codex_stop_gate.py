...
def closeout_reason(root: Path, evidence: Path) -> str:
    ...
    return (
        ...
        "Before acting on this continuation, confirm the evidence still owns an "
        "active run for this session. If a later start superseded and cancelled "
        "it so it no longer owns an active run, do not revive it or call finish; "
        "continue the current active run instead. "
